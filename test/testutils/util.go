...
func CreateLeaderPodsWithInjectedMeta(ctx context.Context, leaderSts appsv1.StatefulSet, k8sClient client.Client, lws *leaderworkerset.LeaderWorkerSet, start int, end int, injectFn func(*corev1.Pod)) error {
	cr, err := revisionutils.NewRevision(ctx, k8sClient, lws, "")
	if err != nil {
		return err
	}

	return createLeaderPodsWithInjectedMeta(ctx, k8sClient, leaderSts, lws, revisionutils.GetRevisionKey(cr), start, end, injectFn)
}
...
// createLeaderPodsWithInjectedMeta creates leader pods with injected metadata using the provided inject function.
func createLeaderPodsWithInjectedMeta(ctx context.Context, k8sClient client.Client, leaderSts appsv1.StatefulSet, lws *leaderworkerset.LeaderWorkerSet, revisionKey string, start, end int, injectFn func(*corev1.Pod)) error {
	for i := start; i < end; i++ {
...
func UpdatePodGroupAtIndex(ctx context.Context, k8sClient client.Client, schedulerProvider schedulerprovider.SchedulerProvider, lws *leaderworkerset.LeaderWorkerSet, groupIndex string) {
	...
	gomega.Expect(CreateLeaderPodsWithInjectedMeta(ctx, leaderSts, k8sClient, lws, startIndex, startIndex+1, injectFn)).To(gomega.Succeed())
}
